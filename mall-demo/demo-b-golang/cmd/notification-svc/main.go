...
import (
	...
	"mall-demo/internal/notification"
...
func main() {
	cfg := config.Load("notification-svc")
	...
	go notification.StartKafkaConsumer(svc, cfg.KafkaBrokers)
	...
	r.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "notification-svc"})
	})
	...
	go func() {
		fmt.Printf("[notification-svc] HTTP listening on %s\n", addr)
	...
	fmt.Println("[notification-svc] shutting down...")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	srv.Shutdown(ctx)
}
